...
class Json
{
    ...
    function encode($arg, $force = true)
    {
        ...
        if ($_force && YF_CHARSET == 'utf-8' && function_exists('json_encode'))
        {
    ...
    function decode($text,$type=0) // 默认type=0返回obj,type=1返回array
    {
        ...
        if (YF_CHARSET === 'utf-8' && function_exists('json_decode'))
        {
            return self::addslashes_deep_obj(json_decode(stripslashes($text),$type));
        }
        ...
        return self::addslashes_deep_obj($result);
    }
