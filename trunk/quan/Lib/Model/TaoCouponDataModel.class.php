<?php
/**
 * TaoCouponDataModel.class.php
 * @copyright			copyright(c) 2011 - 2012 极好居
 * @author				anqiu xiao
 * @contact				QQ:89249294 E-mail:jihaoju@qq.com
 * @date				Mon Jun 18 16:12:05 CST 2012
 */
/**
 * 淘宝优惠券附属表模型类
 */
class TaoCouponDataModel extends Model 
{							
    /**
     * 添加
     * 
    ...
    public function front_hot(array $params=array(), array $limit=array())
    {
    	...
    	$sql .= " LEFT JOIN " . M('tao_coupon')->getTableName() . " AS c ON c.c_id=cd.c_id";
    	$sql .= " LEFT JOIN " . M('tao_shop')->getTableName() . " AS m ON m.id=c.s_id";
    ...
    {
    	$sql = "SELECT cd.c_id,c.title,c.c_type,c.s_title,c.money_max,c.money_reduce,c.money_amount,c.fetched_amount,m.pic_path";
    	$sql .= " FROM " . $this->getTableName() . " AS cd";
    	$sql .= " LEFT JOIN " . M('tao_coupon')->getTableName() . ' AS c ON c.c_id=cd.c_id';
    	$sql .= " LEFT JOIN " . M('tao_shop')->getTableName() . ' AS m ON m.id=c.s_id';
