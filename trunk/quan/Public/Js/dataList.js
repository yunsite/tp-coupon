	...
	if($("#"+module + "_" + name + "_" + id).length == 0)
	{
		...
		input.blur(function(){
			if($.trim(this.value).length > 0)
			{
				...
				submitEdit(obj,group,module,model,pk,id,value,name);
			}
			else
			{
				$(obj).show();
...
function submitEdit(obj,group,module,model,pk,id,val,name)
{
	...
	$.ajax({
		url: '?g='+group+'&m=' + module + '&a=editField',
		type:"POST",
		cache: false,
		data:query,
		dataType:"json",
...
function toggleStatus(obj,id,name)
{
	...
	$.ajax({
		url: '?g='+group+'&m=' + module + '&a=toggleStatus',
		type:"POST",
		cache: false,
		data:query,
		dataType:"json",
		success: function(result){
			if(result.status == 1)
			{
				...
				var src = img.src.replace(query.val + '.gif',result.info + '.gif');
				img.setAttribute('status',result.info);
				img.src = src;
			}
			else
				alert(result.info);
